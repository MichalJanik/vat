...
_vies_fault_re = re.compile(
    r'/(INVALID_INPUT|SERVICE_UNAVAILABLE|MS_UNAVAILABLE|TIMEOUT|SERVER_BUSY)/')
...
class VIESSOAPException(VIESException):
    ...
    def __unicode__(self):
        return '%s - %s (%s)\n%s' % (
            self.code, self.string, self.actor, self.detail)
...
<env:Envelope xmlns:env="http://schemas.xmlsoap.org/soap/envelope/"
 env:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
  <env:Body xmlns:vies="urn:ec.europa.eu:taxud:vies:services:checkVat:types">
    ...
    tries = 0
    response = None
    while response is None or (tries < 5 and 500 <= response.status <= 599):
        if tries > 0:
            time.sleep(tries)
        tries += 1
    ...
    return VIESResponse(country_code, number, request_date, valid, name,
                        address)
