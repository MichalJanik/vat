...
class MemberState(object):
    ...
    def __init__(self, code, iso_alpha_2, name,
        ...
        self.distance_selling_threshold = to_threshold(
            distance_selling_threshold)

        # The small enterprise threshold (a Threshold object)
        self.small_enterprise_threshold = to_threshold(
            small_enterprise_threshold)
    ...
    def reduced_rates(self):
        return tuple(
            [r.rate / 100 for r in _rate_cache.reduced_rates(self.code) if
             r.detail not in ('Super Reduced', 'Parking')])
